...
The Cox proportional hazards (PH) model makes two major assumptions. One of them is that the hazard functions for the survival curves of different strata will be proportional over the period of time, while the other one is the relationship between the log of hazard $h(t)$ and every covariate will be linear. We omit the details for the latter assumption and focus on the first one in this project. Generally speaking, the proportional hazards assumption is significant for the interpretation and the use of PH model. The following describes some graphical analysis methods in conjunction with the interaction test and residuals checking.

**Graphical Approach** One of the most popular strategies for PH assumption checking is to compare the survival curves visually. Recall for a PH model, $S(t|Z = z) = e^{-\int h_0(t)e^{\beta z}dt} = S_0(t)^{e^{\beta z}}$. By using a log-log transformation (i.e., $log\{-logS(t|Z = z)\}$), we will have $$log\{-log\hat{S}(t|Z = z)\} - log\{-log\hat{S}_0(t)\} = \beta,$$ where $Z \in \{0, 1\}$ is an indicator variable, e.g., sex. Such equation indicates two parallel curves if the proportional assumption holds. Figure 4 shows the transformed survival functions estimated by the K-M estimator along with the log of time. 

The other simple way is to compare the differences between the observed and fitted survival functions (Figure 5). 
