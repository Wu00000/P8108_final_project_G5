...
The Cox proportional hazards model makes two major assumptions. One of them is that the hazard functions for the survival curves of different strata will be propotional over the period of time, while the other one is the relationship between the log of hazard $h(t)$ and every covariate will be linear.

**Graphical Approach** One of the most popular strategies for PH assumption checking is to compare the survival curves visually. Recall for a PH model, $S(t|Z = z) = e^{-\int h_0(t)e^{\beta z}dt} = S_0(t)^{e^{\beta z}}$. By using a log-log transformation (i.e., $log\{-logS(t|Z = z)\}$), we will have $$log\{-log\hat{S}(t|Z = z)\} - log\{-log\hat{S}_0(t)\} = \beta,$$ where $Z$ is an indicator variable, e.g., sex. Such equation indicates two parallel curves if the proportional assumption holds.
...
plot(fit_km_sex, fun = "cloglog", col = c("black", "red"), xaxt = "none",
     ...
     main = "Log of Negative Log of Estimated Survival Functios")
